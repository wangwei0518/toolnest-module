...
export class WorkflowTicketsService {
  ...
  async completeNode(ticketId: string, nodeId: string): Promise<Ticket> { const ticket = this.executableTicket(ticketId); const node = this.ticketNode(ticket, nodeId); const definition = this.nodeDefinition(ticket, node); const errors = this.validateValues(definition.form_schema.fields, node.values); if (Object.keys(errors).length) { await this.executeNodeActions(ticket, node, definition, "on_fail"); await this.store.save(); fail(422, Object.values(errors).join("；")); } if (!this.evaluateRule(definition.completion_rule, ticket, node)) { await this.executeNodeActions(ticket, node, definition, "on_fail"); await this.store.save(); fail(422, "完成条件尚未满足"); } node.status = "completed"; node.completed_at = nowIso(); node.outputs = [{ key: "result", value: clone(node.values) }]; this.event("node_completed", `节点「${node.name}」已完成`, node.node_id, ticket.id, ticket.project_id, ticket.milestone_id); await this.executeNodeActions(ticket, node, definition, "on_complete"); await this.notify("node_completed", `节点「${node.name}」已完成`, ticket, node.node_id); const activated = this.activateEdges(ticket); for (const ready of activated) { const readyDefinition = this.nodeDefinition(ticket, ready); await this.executeNodeActions(ticket, ready, readyDefinition, "on_enter"); } if (ticket.node_instances.every((item) => ["completed", "skipped", "cancelled"].includes(item.status))) { ticket.status = "completed"; this.event("ticket_completed", "工单已完成", undefined, ticket.id, ticket.project_id, ticket.milestone_id); await this.notify("ticket_completed", "工单已完成", ticket); } else { ticket.status = "in_progress"; for (const ready of activated) await this.notify("node_ready", `节点「${ready.name}」已进入待处理`, ticket, ready.node_id); } ticket.updated_at = nowIso(); await this.store.save(); return this.getTicket(ticket.id); }

  async blockNode(ticketId: string, nodeId: string, reason: string): Promise<Ticket> { const ticket = this.executableTicket(ticketId); const node = this.ticketNode(ticket, nodeId); const definition = this.nodeDefinition(ticket, node); if (!text(reason)) fail(422, "请填写阻塞原因"); node.status = "blocked"; node.blocked_reason = text(reason); ticket.status = "blocked"; ticket.updated_at = nowIso(); this.event("node_blocked", `节点「${node.name}」已阻塞`, node.node_id, ticket.id, ticket.project_id, ticket.milestone_id, reason); await this.executeNodeActions(ticket, node, definition, "on_block"); await this.notify("ticket_blocked", `工单「${ticket.title}」已阻塞`, ticket, node.node_id); await this.store.save(); return this.getTicket(ticket.id); }

  async reopenTicket(ticketId: string): Promise<Ticket> { const ticket = this.findTicket(ticketId); if (ticket.status === "cancelled") { for (const node of ticket.node_instances) if (node.status === "cancelled") { node.status = "pending"; node.started_at = null; node.completed_at = null; node.blocked_reason = ""; } ticket.status = "in_progress"; const activated = this.activateEdges(ticket, true); for (const ready of activated) await this.executeNodeActions(ticket, ready, this.nodeDefinition(ticket, ready), "on_enter"); this.event("ticket_restarted", "工单已重新启动", undefined, ticket.id, ticket.project_id, ticket.milestone_id); } else { ticket.status = "in_progress"; const reopened = ticket.node_instances.filter((node) => node.status === "blocked"); for (const node of reopened) { node.status = "ready"; node.blocked_reason = ""; await this.executeNodeActions(ticket, node, this.nodeDefinition(ticket, node), "on_enter"); } this.event("ticket_reopened", "工单已重新打开", undefined, ticket.id, ticket.project_id, ticket.milestone_id); } ticket.updated_at = nowIso(); await this.notify("ticket_reopened", "工单已重新打开", ticket); await this.store.save(); return this.getTicket(ticket.id); }
  ...
  async actionProviders(): Promise<Array<Record<string, unknown>>> {
    const channels = await this.notificationChannelOptions();
    return [{
      key: "notification",
      label: "平台通知",
      description: "通过 ToolNest 通知中心投递，不在模块内直接保存或发送渠道凭据。",
      events: ["on_enter", "on_complete", "on_fail", "on_block"],
      notification_channels: [
        { key: "default", label: channels.default_label, channels: channels.default_channels },
  ...
  private async notificationChannelOptions(): Promise<{ default_label: string; default_channels: string[]; configured_channels: Array<{ key: string; label: string; configured: boolean }> }> {
    const fallbackChannels = ["web_internal"];
    const labels: Record<string, string> = { web_internal: "站内通知", qqbot: "QQ 机器人", email: "邮件", webhook: "Webhook", gotify: "Gotify", telegram: "Telegram" };
    const fallback = { default_label: "平台默认通知", default_channels: fallbackChannels, configured_channels: [] };
    if (!this.platformApiUrl || !this.platformToken) return fallback;
    try {
      const response = await fetch(`${this.platformApiUrl}/internal/modules/${encodeURIComponent(this.moduleId)}/notification-channels`, { headers: { "x-toolnest-internal-token": this.platformToken }, signal: AbortSignal.timeout(3000) });
      if (!response.ok) return fallback;
      const payload = asObject(await response.json());
      const data = asObject(payload.data ?? payload);
      const defaultChannels = [...new Set(asArray(data.default_channels).map(text).filter(Boolean))];
      const configuredChannels = asArray(data.channels).map((item) => {
        const value = asObject(item);
        const key = text(value.key);
        return { key, label: text(value.label) || labels[key] || key, configured: value.configured === true };
      }).filter((item) => item.configured && SUPPORTED_NOTIFICATION_CHANNELS.has(item.key));
      const defaults = defaultChannels;
      return {
        default_label: "平台默认通知",
        default_channels: defaults,
        configured_channels: configuredChannels.filter((item) => !defaults.includes(item.key)),
      };
    } catch {
      return fallback;
    }
  ...
  private async executeNodeActions(ticket: Ticket, node: TicketNode, definition: WorkflowNode, event: string): Promise<void> {
    for (const rawAction of definition.actions) {
      const action = asObject(rawAction);
      if (text(action.key) !== "notification" || text(action.event) !== event) continue;
      const execution: ActionExecution = { id: id(), ticket_id: ticket.id, node_id: node.node_id, provider_key: "notification", event, status: "failed", input: clone(action), output: {}, error: null, created_at: nowIso(), completed_at: null };
      try {
        if (!this.platformApiUrl || !this.platformToken) throw new Error("平台通知服务未配置");
        const custom = text(action.notification_channel_mode) === "custom";
        const requestedChannels = [...new Set(asArray(action.notification_channels).map(text).filter(Boolean))];
        const channels = requestedChannels.filter((channel) => SUPPORTED_NOTIFICATION_CHANNELS.has(channel));
        if (custom && !channels.length) throw new Error("节点动作未选择有效的通知渠道");
        const eventLabel: Record<string, string> = { on_enter: "已进入", on_complete: "已完成", on_fail: "执行失败", on_block: "已阻塞" };
        const title = text(action.label) || `节点「${node.name}」${eventLabel[event] ?? "发生变化"}`;
        const requestBody: Record<string, unknown> = { title, summary: ticket.number, content: ticket.note || title, level: event === "on_complete" ? "success" : event === "on_block" || event === "on_fail" ? "warning" : "info", event_type: `workflow_tickets.node_action.${event}`, source_type: "ticket", source_id: ticket.id, related_url: `/modules/${this.moduleId}/tickets/${ticket.id}` };
        if (custom) requestBody.channels = channels;
        const response = await fetch(`${this.platformApiUrl}/internal/modules/${encodeURIComponent(this.moduleId)}/notifications`, { method: "POST", headers: { "content-type": "application/json", "x-toolnest-internal-token": this.platformToken }, body: JSON.stringify(requestBody), signal: AbortSignal.timeout(10000) });
        const payload = asObject(await response.json().catch(() => ({})));
        if (!response.ok) throw new Error(text(payload.message) || `平台通知发送失败（${response.status}）`);
        execution.status = "succeeded";
        execution.output = {
          notification_id: text(payload.notification_id) || null,
          deliveries: asArray(payload.deliveries).map((item) => {
            const delivery = asObject(item);
            return { channel: text(delivery.channel ?? delivery.channel_type), status: text(delivery.status) };
          }),
        };
      } catch (error) {
        execution.error = error instanceof Error ? error.message : String(error);
