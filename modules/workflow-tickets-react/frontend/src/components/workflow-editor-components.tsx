...
export function WorkflowFormDesignerDialog({ open, onOpenChange, node, initialFieldId, variables = [], onApply }: {
  ...
  <Dialog open={advancedOpen} onOpenChange={setAdvancedOpen}>
    <DialogContent className="flex h-[min(48rem,calc(100dvh-2rem))] max-h-[calc(100dvh-2rem)] max-w-5xl flex-col overflow-hidden">
      ...
      <div className="grid min-h-0 min-w-0 flex-1 gap-4 overflow-hidden md:grid-cols-[minmax(0,1fr)_minmax(14rem,0.45fr)]">
        <Field className="min-h-0 min-w-0"><FieldLabel>{advancedKind === "execution" ? "脚本内容" : "模板内容"}</FieldLabel><Textarea ref={advancedTextareaRef} className="h-72 min-h-0 min-w-0 max-h-full [field-sizing:fixed] overflow-y-auto font-mono" rows={14} value={advancedDraft} onChange={(event) => setAdvancedDraft(event.target.value)} placeholder="输入模板内容，或从右侧插入动态字段" /></Field>
