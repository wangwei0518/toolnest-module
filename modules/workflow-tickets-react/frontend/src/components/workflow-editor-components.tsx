...
export type FormVariable = { label: string; value: string; group: string; source_type?: string };
...
export function WorkflowFormDesignerDialog({ open, onOpenChange, node, initialFieldId, variables = [], onApply }: {
        ...
        <aside className="flex min-h-0 flex-col overflow-hidden p-4">
          {active ? <><div className="mb-3 flex items-center justify-between"><div><h3 className="text-sm font-semibold">字段配置</h3><p className="text-xs text-muted-foreground">{fieldTypeLabel(active.type)}</p></div><Button type="button" variant="destructiveSolid" size="icon-sm" aria-label={`删除字段 ${active.label || "未命名字段"}`} onClick={() => requestFieldDelete(active.id)}><RiDeleteBinLine /></Button></div>
            <div className="min-h-0 flex-1 space-y-4 overflow-y-auto pr-1"><FieldGroup>
              ...
              {referenceVariables.length > 0 || active.reference ? <><div className="tn-workflow-tickets-form-config-divider" aria-hidden="true" /><Field><FieldLabel>字段引用</FieldLabel><Select value={active.reference?.path ?? "__none"} onValueChange={(path) => updateReference(active.id, path ?? "__none")}><SelectTrigger className="w-full"><SelectValue placeholder="选择前序节点字段或输出" /></SelectTrigger><SelectContent><SelectItem value="__none">不引用字段</SelectItem>{referenceVariables.map((item) => <SelectItem key={item.value} value={item.value}>{item.label} · {fieldTypeLabel(item.source_type ?? "output")}</SelectItem>)}</SelectContent></Select><FieldDescription>引用后会在节点进入时保存一份当前值，并自动以只读方式展示。</FieldDescription></Field></> : null}
  ...
  <Dialog open={advancedOpen} onOpenChange={setAdvancedOpen}>
    <DialogContent className="max-h-[min(48rem,calc(100dvh-2rem))] max-w-5xl overflow-hidden">
      <DialogHeader><DialogTitle>{advancedKind === "execution" ? "执行脚本模板" : "动态默认值"} · {fields.find((field) => field.id === advancedFieldId)?.label ?? "字段模板"}</DialogTitle><DialogDescription>支持 <code>{"{{ 字段路径 }}"}</code>，也可以从右侧插入工单或前序节点变量。</DialogDescription></DialogHeader>
      <div className="grid min-h-0 gap-4 md:grid-cols-[minmax(0,1fr)_minmax(14rem,0.45fr)]">
        <Field className="min-h-0"><FieldLabel>{advancedKind === "execution" ? "脚本内容" : "模板内容"}</FieldLabel><Textarea ref={advancedTextareaRef} className="min-h-72 font-mono" rows={14} value={advancedDraft} onChange={(event) => setAdvancedDraft(event.target.value)} placeholder="输入模板内容，或从右侧插入动态字段" /></Field>
        <div className="grid min-h-0 content-start gap-2"><Input value={advancedQuery} onChange={(event) => setAdvancedQuery(event.target.value)} placeholder="搜索可用变量" aria-label="搜索可用变量" /><div className="max-h-72 space-y-3 overflow-y-auto">{[...new Set(templateVariables.map((item) => item.group))].map((group) => { const items = templateVariables.filter((item) => item.group === group && `${item.label} ${item.value} ${item.group}`.toLowerCase().includes(advancedQuery.trim().toLowerCase())); return items.length ? <section key={group}><h3 className="mb-1 text-xs font-semibold text-muted-foreground">{group}</h3><div className="grid gap-1">{items.map((item) => <Button key={item.value} className="h-auto justify-start whitespace-normal px-2 py-1.5 text-left" size="sm" variant="ghost" onClick={() => insertVariable(item)}><span><strong className="block text-xs">{item.label}</strong><code className="text-[0.65rem] text-muted-foreground">{item.value}</code></span></Button>)}</div></section> : null; })}</div></div>
      </div>
...
export type ActionProvider = { key: string; label: string; description?: string; events: string[] };
...
export function WorkflowActionEditor({ actions, providers, onChange }: { actions: WorkflowAction[]; providers: ActionProvider[]; onChange: (actions: WorkflowAction[]) => void }) {
  ...
  const add = () => { const provider = providers[0]; if (provider) onChange([...actions, { key: provider.key, event: provider.events[0] ?? "on_complete", label: "" }]); };
