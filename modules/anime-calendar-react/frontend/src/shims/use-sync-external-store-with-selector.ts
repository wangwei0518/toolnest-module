...
export function useSyncExternalStoreWithSelector(
  ...
  const instance = useRef<{ hasValue: boolean; value: unknown }>({ hasValue: false, value: undefined });
  const [getSelection, getServerSelection] = useMemo(() => {
    ...
    const memoizedSelector = (snapshot: unknown) => {
      if (!hasMemo) {
        ...
        if (isEqual && instance.current.hasValue && isEqual(instance.current.value, nextSelection)) {
    ...
    return [() => memoizedSelector(getSnapshot()), getServerSnapshot ? () => memoizedSelector(getServerSnapshot()) : undefined] as const;
  }, [getServerSnapshot, getSnapshot, isEqual, selector]);
  const value = useSyncExternalStore(subscribe, getSelection, getServerSelection);
  useEffect(() => { instance.current.hasValue = true; instance.current.value = value; }, [value]);
  return value;
}
