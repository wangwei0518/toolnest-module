...
export function AnimeDetailSheet({ item, onOpenChange }: { item: AnimeListItem | null; onOpenChange: (open: boolean) => void }) {
  ...
  return <Sheet open={Boolean(item)} onOpenChange={onOpenChange}><SheetContent className="tn-anime-detail-sheet">
    ...
    {!current && detail.isLoading ? <div className="tn-anime-detail-sheet__loading"><Skeleton className="h-64 w-full" /><Skeleton className="h-6 w-2/3" /><Skeleton className="h-24 w-full" /></div> : detail.isError ? <Alert variant="destructive"><AlertTitle>详情加载失败</AlertTitle><AlertDescription><Button size="sm" variant="outline" onClick={() => void detail.refetch()}>重试</Button></AlertDescription></Alert> : current ? <div className="tn-anime-detail">
      <section className="tn-anime-detail__hero"><img src={current.cover_url || fallbackCover(current)} alt={displayTitle(current)} onError={(event) => { event.currentTarget.src = fallbackCover(current); }} /><div><div className="flex flex-wrap gap-2"><Badge className="tn-anime-detail__media-badge">{mediaLabels[current.media_type]}</Badge><Badge variant="secondary">{statusLabels[current.status]}</Badge>{current.score ? <Badge variant="outline">★ {current.score}</Badge> : null}</div><p>{current.cour_label}</p><h2>{displayTitle(current)}</h2>{current.title_original && current.title_original !== displayTitle(current) ? <span>{current.title_original}</span> : null}<strong>{airInfo(current)}</strong><div className="flex flex-wrap gap-2"><Button onClick={() => mark.mutate({ item: current, mark: "watching" })}>{current.mark_type === "watching" ? <RiHeartFill data-icon="inline-start" /> : <RiHeartLine data-icon="inline-start" />}{current.mark_type === "watching" ? "取消关注" : "关注"}</Button>{current.external_url ? <Button variant="outline" nativeButton={false} render={<a href={current.external_url} target="_blank" rel="noreferrer" />}><RiExternalLinkLine data-icon="inline-start" />Bangumi</Button> : null}<Button size="icon" variant="outline" aria-label={current.mark_type === "ignored" ? "取消屏蔽" : "屏蔽"} onClick={() => mark.mutate({ item: current, mark: "ignored" })}><RiEyeOffLine /></Button></div></div></section>
