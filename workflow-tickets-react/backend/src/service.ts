...
function ticketRead(ticket: Ticket, workflows: Workflow[]): Ticket {
...
export class WorkflowTicketsService {
  ...
  async overview(): Promise<Record<string, unknown>> {
    ...
    return { todo: todo.map((ticket) => ticketRead(ticket, this.data.workflows)), in_progress: inProgress.map((ticket) => ticketRead(ticket, this.data.workflows)), today: today.map((ticket) => ticketRead(ticket, this.data.workflows)), timeline: this.timeline(undefined, 8), counts, inbox_count: this.data.inbox_items.filter((item) => item.status === "inbox").length, project_counts: projectCounts };
  }
  ...
  timelineActivity(since?: string, until?: string, timeZone = "UTC"): TimelineActivity {
  ...
  listTickets(query: Record<string, unknown> = {}): Page<Ticket> {
    ...
    if (view === "mine") items = items.filter((ticket) => ticket.owner_id === text(query.owner_id));
    ...
    const page = Math.max(1, Number(query.page) || 1); const pageSize = Math.min(100, Math.max(1, Number(query.page_size) || 100)); const sorted = sortedUpdated(items); return { items: sorted.slice((page - 1) * pageSize, page * pageSize).map((ticket) => ticketRead(ticket, this.data.workflows)), total: sorted.length, page, page_size: pageSize };
  }

  getTicket(ticketId: string): Ticket {
    const ticket = clone(this.findTicket(ticketId)); for (const node of ticket.node_instances) for (const [fieldId, resource] of Object.entries(node.resources)) { const current = this.data.temporary_resources.find((item) => item.token === (text(resource.url).split("/").filter(Boolean).at(-1) ?? "")); if (current) node.resources[fieldId] = this.resourceRead(current); } return ticketRead(ticket, this.data.workflows);
  }
  ...
  listInbox(includeArchived = false): InboxItem[] { return sortedUpdated(this.data.inbox_items.filter((item) => includeArchived || ["inbox", "completed"].includes(item.status))); }
  async createInbox(body: Record<string, unknown>, actor = "当前用户"): Promise<InboxItem> { const created = nowIso(); const item: InboxItem = { id: id(), title: text(body.title), note: text(body.note), status: "inbox", owner_id: text(body.owner_id) || null, owner_name: text(body.owner_name) || actor, due_at: text(body.due_at) || null, reminder_at: text(body.reminder_at) || null, ticket_id: null, created_at: created, updated_at: created }; if (!item.title) fail(422, "事项标题不能为空"); this.data.inbox_items.unshift(item); await this.store.save(); return clone(item); }
  async updateInbox(itemId: string, body: Record<string, unknown>): Promise<InboxItem> { const item = this.findInbox(itemId); if (body.completed !== undefined) item.status = Boolean(body.completed) ? "completed" : "inbox"; if (body.title !== undefined) item.title = text(body.title); item.updated_at = nowIso(); await this.store.save(); return clone(item); }
  async archiveInbox(itemId: string): Promise<InboxItem> { const item = this.findInbox(itemId); if (item.status === "converted") fail(409, "已转换事项不能归档"); item.status = "archived"; item.updated_at = nowIso(); await this.store.save(); return clone(item); }
  async deleteInbox(itemId: string): Promise<void> { this.findInbox(itemId); this.data.inbox_items = this.data.inbox_items.filter((item) => item.id !== itemId); await this.store.save(); }
 ...
 async runDueReminders(): Promise<number> { let count = 0; const now = Date.now(); for (const ticket of this.data.tickets) if (["draft", "in_progress", "blocked"].includes(ticket.status) && ticket.reminder_at && !ticket.reminder_sent_at && new Date(ticket.reminder_at).valueOf() <= now) { ticket.reminder_sent_at = nowIso(); this.event("ticket_reminder_sent", "已发送工单提醒", undefined, ticket.id, ticket.project_id, ticket.milestone_id); await this.notify("ticket_reminder", `工单「${ticket.title}」提醒`, ticket); count++; } if (count) await this.store.save(); return count; }
  ...
  async exportData(): Promise<Record<string, unknown>> { const data = clone(this.data) as unknown as Record<string, unknown>; const attachments = this.data.attachments.map((attachment) => ({ id: attachment.id, ticket_id: attachment.ticket_id, ticket_node_id: attachment.ticket_node_id, field_id: attachment.field_id, name: attachment.filename, filename: attachment.filename, mime_type: attachment.mime_type, size: attachment.size, content_base64: "" })); for (const item of attachments) { try { item.content_base64 = (await readFile(this.getAttachment(item.id).path)).toString("base64"); } catch { /* omit missing contents */ } } data.attachments = attachments; return { schema_version: 2, exported_at: nowIso(), data }; }
  async importData(body: Record<string, unknown>): Promise<Record<string, unknown>> {
    ...
    for (const key of ["workflows", "tickets", "timeline", "projects", "milestones", "inbox_items", "schedules", "schedule_runs", "saved_views", "automations", "resources", "action_executions"] as const) {
  ...
  private findInbox(itemId: string): InboxItem { const item = this.data.inbox_items.find((value) => value.id === itemId); if (!item) fail(404, "收件箱事项不存在"); return item; }
