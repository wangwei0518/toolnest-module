...
export class WorkflowTicketsService {
  ...
  async updateTicket(ticketId: string, body: Record<string, unknown>): Promise<Ticket> {
    ...
    this.resolveScope(text(body.project_id) || ticket.project_id, text(body.milestone_id) || ticket.milestone_id); if (body.project_id !== undefined) ticket.project_id = text(body.project_id) || null; if (body.milestone_id !== undefined) ticket.milestone_id = text(body.milestone_id) || null;
