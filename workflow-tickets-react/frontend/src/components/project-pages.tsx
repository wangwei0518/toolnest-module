...
function ProjectCard({ project, milestones, router, expanded, onToggle }: { project: Project; milestones: Milestone[]; router: Router; expanded: boolean; onToggle: () => void }) {
  ...
  return <Card className={"gap-0 border border-border py-0 " + (expanded ? "xl:col-span-2" : "")}>
    <div className="grid gap-4 p-4 sm:p-5">
      <header className="flex min-w-0 items-center gap-3">
        <Button type="button" size="icon-sm" variant="ghost" aria-label={expanded ? "收起项目" : "展开项目"} onClick={onToggle}><RiArrowDownSLine className={expanded ? "transition-transform" : "-rotate-90 transition-transform"} /></Button>
    ...
    {expanded ? <div className="grid gap-5 border-t p-4 sm:p-5 xl:grid-cols-[minmax(0,1fr)_minmax(18rem,0.36fr)]">
...
export function ProjectsPage({ api, router }: PageProps) {
  ...
  const [projects, setProjects] = useState<Project[]>([]); const [projectMilestones, setProjectMilestones] = useState<Record<string, Milestone[]>>({}); const [keyword, setKeyword] = useState(""); const [status, setStatus] = useState("all"); const [loading, setLoading] = useState(true); const [error, setError] = useState(""); const [createOpen, setCreateOpen] = useState(false); const [expandedProjectId, setExpandedProjectId] = useState<string | null>(null);
