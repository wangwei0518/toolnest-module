...
function collectItemLabels(node: React.ReactNode, labels: Map<string, string>) {
  if (Array.isArray(node)) {
    node.forEach((child) => collectItemLabels(child, labels))
    return
  }
  if (!React.isValidElement(node)) return
...
function Select({ children, itemToStringLabel, ...props }: SelectProps) {
  const labels = new Map<string, string>()
  collectItemLabels(children, labels)
  return (
    <SelectPrimitive.Root
      {...props}
      itemToStringLabel={itemToStringLabel ?? ((item: string) => labels.get(item) ?? item)}
    >
      {children}
    </SelectPrimitive.Root>
  )
